...
public class IllusionFade : MonoBehaviour {
	...
	void Start () {
        isFading = true;
        alpha = 0.0f;
        gameObject.GetComponent<SpriteRenderer>().color = new Color(gameObject.GetComponent<SpriteRenderer>().color.r, gameObject.GetComponent<SpriteRenderer>().color.g, gameObject.GetComponent<SpriteRenderer>().color.b, 0.0f);
	...
	void Update () {
        ...
        if(isFading == true)
        {
            StartCoroutine(FadeTo(alpha, 5.0f));
            isFading = false;
        }
        else
        {
